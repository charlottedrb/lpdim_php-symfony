    ...
    <div class="flex flex-col mb-2">
        <div class="relative">
            <input type="string"
                   ...
                   id="name" aria-describedby="name" placeholder="Name"
                   name="name"
                   value="<?php if(!is_array($game)): echo $game->getName(); else: echo ""; endif; ?>">
        </div>
    </div>
    <div class="flex flex-col mb-2">
        <div class="relative">
            <input type="string"
                   ...
                   name="image"
                   value="<?php if(!is_array($game)): echo $game->getImage(); else: echo ""; endif; ?>">
        </div>
    </div>
    <div class="flex flex-col mb-2">
        <div class="relative">
            <select name="owner" class="rounded-lg border-transparent flex-1 border border-gray-300 w-full py-2 px-4 bg-white text-gray-700 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent">
                <?php foreach($players as $player): ?>
                    <option class="" value="<?= $player->getUsername() ?>"><?= $player->getUsername() ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    </div>
